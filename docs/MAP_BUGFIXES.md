...
3. Naver map rendering can require runtime script/image/connect assets beyond the SDK URL itself. The SDK loads style JSONP from `nrbe.pstatic.net`, so allowing only the main SDK host can make SDK initialization partially succeed while tile styles fail.
...
- `next.config.mjs`
  - Expanded Naver Maps CSP asset coverage for Naver/NCP runtime script, image, and connect hosts.
