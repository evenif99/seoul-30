...
- Naver Maps 핀포인트 (Seoul 경계 검증, 위성/하이브리드 뷰, 미니맵, 최대 30개 핀)
- ko/en 이중 언어 (next-intl v4, 쿠키 기반)
- PWA (오프라인 페이지, 웹 푸시 알림, 홈 설치, 4-tier 서비스 워커 캐시)
- 익명 장소 평점 (👍/👎, Prisma + Neon)
- 북마크 / 최근 본 장소 (localStorage, 실 API 장소 포함)
- 근처 장소 추천 섹션
- schema.org JSON-LD 구조화 데이터 (TouristAttraction)
...
사용자 요청
  └─ app/api/places/route.ts
       ...
       ├─ TourAPI 이미지 보강 (imageUrl 없는 상위 30개)
...
```bash
npm run test          # Vitest unit + component (99개)
npm run test:e2e      # Playwright E2E (13개)
