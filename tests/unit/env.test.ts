...
describe('validateEnv', () => {
  ...
  it('throws when ENABLE_CULTURE_EVENTS_API=true but TOUR_API_KEY is missing', () => {
    vi.stubEnv('DATABASE_URL', 'postgresql://localhost/test')
    vi.stubEnv('ENABLE_CULTURE_EVENTS_API', 'true')
    vi.stubEnv('SEOUL_OPEN_API_KEY', 'seoul_test')
    vi.stubEnv('TOUR_API_KEY', '')
    expect(() => validateEnv()).toThrow('TOUR_API_KEY is required')
  })
